	...
	module.exports = new Class({
		...
		, executeTTLCheck: function() {
			for (let queue of this.queues.values()) {
				while (queue.length && queue.getLast().isExpired(this.ttl)) {
					...
					for (let q of queues) q.remove(q.id);
				}
			}
		}
		...
		, setUpPool: function(node) {
			...
			node.pools.forEach((poolName) => {
				...
				this.pools.set(poolName).nodeCount++;
			});
			...
			node.on('connection', (connection) => {
				...
				connection.on('idle', () => {
					...
					if (this.queues.has(node.compositeName) && this.queues.get(node.compositeName).length) {
						...
						for (let queue of this.queueMap.get(request.pool)) queue.remove(request.id);
				...
				// it ends
				connection.once('end', () => {
					for (let poolName of node.pools) this.pools.get(poolName).remove(connection.id);
				});
			});
		}
		...
		, getConnection: function(poolName) {
			...
			else if (!this.queueMap.has(poolName) || !this.queueMap.get(poolName).length) return Promise.reject(new Error('Cannot get connection, no host is sesrving the pool «'+poolName+'»!'));
