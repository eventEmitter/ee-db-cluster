!function(){
	...
	module.exports = new Class({
		...
		, getConnection: function(){
			...
			// queue request
			if (!this.queue[mode].add(function(err, connection){
				if (err) callback(err);
				else {
					connection.removeFromPool();
					callback(null, connection);
				}
			}.bind(this))) callback(new Error('Failed to add the connection request to the queue, overflow!').setName('OverflowException'));
