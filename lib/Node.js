(function(){
	...
	module.exports = new Class({
		...
		// mode of operation
		, validModes: ['readonly', 'readwrite', 'writeonly']
		...
		// indicates which queue to use for the next query on this host
		, queryMode: {
			get: function() {
				if (this.mode == 'readonly') return this.mode;
				else if (this.mode === 'writeonly') return this.mode;
				else {
					this.queryModeStatus = !this.queryModeStatus;
					return this.queryModeStatus ? 'readonly' : 'writeonly';
				}
			}
		}


		// private storage for the flag above
		, queryModeStatus: false




		// if the host has an user defined id each second query
		// should be executed for the id specific query, this flag 
		// indicates for which queue this host is currently used
		, isIdQuery: {
			get: function() {
				if (this.hasHostId && !this.idOnly) this.idQueryStatus = !this.idQueryStatus;
				return this.hasHostId ? (this.idOnly || this.idQueryStatus) : false;
			}
		}
		...
		// indicates if the host should only execute queries
		// directed at its user defines host id
		, idOnly: false


		// indicates if this host has a user defined id
		, hasHostId: false
		...
		, idle: {
			get: function() {
				return this.maxConnections/(this.idleCount+this.creatingCount);
			}
		}
		...
		, count: {
			get: function() {
				return this.connections.length+this.creatingCount;
			}
		}
		...
		, init: function(config) {
			...
			// user defined ids are strings, auto created ones are symbols
			this.hasHostId = config.hasHostId = type.string(this.id);
		...
		, createConnection: function() {
			...
			if (!this.ended && this.idle < this.idlePercent && this.count < this.maxConnections) {
		...
		, executeCreateConnection: function() {
			var connection = new this.ConnectionConstructor(this.config, this.getConnectionId(), this);
		...
		, handleConnectivityProblem: function() {
			...
			if (!this.ended && !this.errorChecking && (Date.now()-this.errorCheckInterval) > this.lastErrorCheck) {
				...
				if (!this.connections.length) this.createConnection();
					...
					let checkTimout = setTimeout(function() {
						...
						this.resetNode();
					}.bind(this), this.errorCheckTimeout);
